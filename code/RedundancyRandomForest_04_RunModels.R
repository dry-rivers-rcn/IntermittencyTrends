## RedundancyRandomForest_04_RunModels.R
...
tune_res_all <- readr::read_csv(file.path("results", "RedundancyRandomForest_03_TuneHyperparameters.csv"))

## loop through metrics and regions
# choose number of predictors - same as RedundancyRandomForest_03_...
...
for (m in metrics){
  ...
  for (r in regions){
    ...
    rf_var_m_r <-
      file.path("results", paste0("RandomForestRedundancy_01_PreliminaryVariableImportance_", m, "_", gsub(" ", "", r, fixed = TRUE), ".csv")) %>% 
      readr::read_csv() %>% 
...
fit_data_out %>% 
  ...
  readr::write_csv(file.path("results", "RedundancyRandomForest_04_RunModels_Predictions.csv"))

fit_rf_imp %>% 
  readr::write_csv(file.path("results", "RedundancyRandomForest_04_RunModels_VariableImportance.csv"))

fit_pdp_out %>% 
  readr::write_csv(file.path("results", "RedundancyRandomForest_04_RunModels_PartialDependence.csv"))
