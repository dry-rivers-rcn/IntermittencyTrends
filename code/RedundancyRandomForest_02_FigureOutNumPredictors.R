## RedundancyRandomForest_02_FigureOutNumPredictors.R
#' This script is intended to figure out the parsimonious number of predictors to use
#' in random forest models.
#' 
#' Input variables will be selected using the output from RedundancyRandomForest_01_PreliminaryVariableImportance.R
#' 
...
# save results
fit_predtest %>% 
  readr::write_csv(path = file.path("results", "RedundancyRandomForest_02_FigureOutNumPredictors.csv"))
...
ggplot(fit_predtest, aes(x = n, y = OOBmse)) + 
  ...
  labs(title = "Random forest MSE as a function of number of predictors",
       subtitle = "National model for each metric; potential redundant gages removed") +
  ggsave(file.path("figures_manuscript", "RedundancyRandomForest_MSEvNumberPredictors.png"),
