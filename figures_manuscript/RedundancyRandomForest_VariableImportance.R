## RedundancyRandomForest_VariableImportance.R
...
# variable importance
rf_imp <- 
  readr::read_csv(file.path("results", "RedundancyRandomForest_04_RunModels_VariableImportance.csv")) %>% 
...
p_nat_zff <-
  ...
  scale_fill_manual(drop = F,
                    ...
                    labels = c("Climate" = "Climate", 
                               "Physiography" = "Physiography",
                               "Land Use" = "Land/Water Use")) +
  scale_y_continuous(name = "MSE Increase [%]", labels = scales::percent_format(accuracy = 2)) +
...
((p_nat_anf + p_nat_p2z + p_nat_zff) + 
  ...
  ggsave(file.path("figures_manuscript", "RedundancyRandomForest_VariableImportance.png"),
