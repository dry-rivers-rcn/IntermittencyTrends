## RedundancyRandomForest_Validation.R
...
# This model is for predicting annual values after removing potentially redundant
# gages, which is how it differs from the script RandomForest_Validation.R
...
gage_sample_annual <-
  ...
  subset(!(gage_ID %in% redundant_gages$gage_ID))
...
rf_all <-
  readr::read_csv(file.path("results", "RedundancyRandomForest_04_RunModels_Predictions.csv")) %>% 
...
rf_fit_regional <-
  ...
  dplyr::summarize(Rsq = round(R2(predicted, observed), 2),
...
rf_fit %>% 
  readr::write_csv(file.path("figures_manuscript", "RedundancyRandomForest_Validation-FitTable-Test.csv"))
...
rf_fit_train %>% 
  readr::write_csv(file.path("figures_manuscript", "RedundancyRandomForest_Validation-FitTable-Train.csv"))
...
#   ggsave(file.path("figures_manuscript", "RedundancyRandomForest_Validation-CompareNationalRegionFit.png"),
...
((p_nat_afnf_train + p_nat_p2z_train + p_nat_zff_train +
  ...
  ggsave(file.path("figures_manuscript", "RedundancyRandomForest_Validation-National-Train+Test.png"),
